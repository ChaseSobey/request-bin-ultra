...
async function insertOne(dbName, collectionName, jsonObj) {
  ...
  let insertedObjInfo = await conn.db(dbName).collection(collectionName).insertOne(jsonObj);
  conn.close();
...
function stringIdToMongoIdObject(objectIdString) {
