...
async function deleteObjects(dbName, collectionName, jsonDeletionCriteria) {
  ...
  await conn
    .db(dbName)
    .collection(collectionName)
    .deleteMany(jsonDeletionCriteria);
  conn.close();
}
...
async function getObjectsById(stringIds) {
  const mongoIds = stringIds.map((stringId) => _stringToObjectId(stringId));
  const mongoObjs = await Promise.all(
    mongoIds.map(async (id) => {
      const result = await this.findDbObj("request-bin-ultra", "requests", {
        _id: id,
      });
      return result;
    })
